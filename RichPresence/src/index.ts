...
function getPurpleStatus(enabled: boolean): any {
  if (!enabled) return undefined;
  return {
    emoji: "\uD83D\uDD2E",
    text: "Purple",
    tooltip: "Purple Streaming Mode"
  };
}
...
async function sendRequest(activity: Activity | null): Promise<Activity | null> {
  ...
  const isStreamingEnabled = activity.type === ActivityTypes.STREAMING && Boolean(activity.streaming_url);
  ...
  if (isStreamingEnabled) {
    activity.purple_status = getPurpleStatus(true);
  } else {
    delete activity.purple_status;
  }
